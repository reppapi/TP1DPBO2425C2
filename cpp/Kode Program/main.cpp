...
// Fungsi buat liat semua barang (VERSI DIPERBAIKI)
void lihatSemua() {
    ...
    // Header tabel
    cout << "\nID\tNama\t\tJenis\t\tHarga\t\tGambar\n";
    cout << "-------------------------------------------------------------\n";

    // Loop untuk menampilkan setiap data dalam format tabel
    for (int i = 0; i < jumlahData; i++) {
        cout << barang[i].getId() << "\t"
             << barang[i].getName() << "\t\t"
             << barang[i].getType() << "\t\t"
             << "Rp" << fixed << setprecision(1) << barang[i].getPrice() << "\t\t" // Format harga
             << barang[i].getImagePath() << endl;
    }
}
